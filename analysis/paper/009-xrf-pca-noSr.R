...
xrf <-
  ...
  dplyr::select(reading_no, site_id, sample_id, hue, material, `Mg`, `Al`, `Si`, `K`, `Ca`, `Fe`, `Zr`, `Ti`)
...
xrf_long <- xrf %>% 
  dplyr::select(cluster,`Mg`, `Al`, `Si`, `K`, `Ca`, `Fe`, `Zr`, `Ti`) %>% 
...
ggsave("009-xrf-pca-load-scree-noSr.png",
       fig1,
       device = "png",
...
ggsave("009-xrf-kmeans-boxplot-noSr.png",
       fig3,
       device = "png",
       here::here("analysis/figures/"),
       scale = 1, 
