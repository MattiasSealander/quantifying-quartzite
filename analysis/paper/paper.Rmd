...
vis.dark <- Points.nir %>% 
  filter(hue == "Dark") %>% 
  select(c(1:27),c(78:428))

spec.vd <- 
      as_spectra(vis.dark, name_idx = 1, meta_idxs = 2:27)
...
spec.vl <- 
  as_spectra(vis.light, name_idx = 1, meta_idxs = 2:27)
...
spec.vt <- 
  as_spectra(vis.translucent, name_idx = 1, meta_idxs = 2:27)
...
spec.vw <- 
  as_spectra(vis.white, name_idx = 1, meta_idxs = 2:27)
...
No distinct peaks occurred in the 600 – 1 000 nm band, and thus the analysis focused on the 1 000 – 2 500 nm band to lessen the impact of noise. In the case of NIRS most features relate to the presence of molecular water in the material [@hunt77; @sciuto_etal19], these mainly occur at around 1 400 nm and 1 900 nm and can be seen in the average spectra (fig. \@ref(fig:nir-spectra)). Peaks also occur at 2 200 nm and 2 350 nm, where the former has been related to combination vibrations of Al-OH and the latter could possibly be related to CO^3^ [@clark_etal90]. While the dark group features a higher overall absorbance the peaks are less distinct. The same can be said for the translucent and white group, where the water peak at 1 900 nm is the most pronounced. The absorbance in the translucent group is slightly higher in the lower band regions than compared to the white group, but with decreased absorbance in the higher regions. While a fifth peak at 2 450 nm can be difficult to discern in most cases....

The PCA demonstrates a similar pattern to the one observed in the visual band (fig. \@ref(fig:nir-pca)). Again the first two components explain most of the variance in the model, ~ 99.9 %. While the darker materials remain closely grouped in the right side of the score plot, the *light* cluster seems to split into two separate groups. The upper left group houses the main bulk of the *brecciated quartz*, as well as the almost translucent and opaque material with a beige tint. The bottom left group instead features the majority of the white *quartz-coloured* material. 
...
spec.l <- 
  as_spectra(light, name_idx = 1, meta_idxs = 2:27)
...
spec.t <- 
  as_spectra(translucent, name_idx = 1, meta_idxs = 2:27)
...
spec.w <- 
  as_spectra(white, name_idx = 1, meta_idxs = 2:27)

#Prepare a layout with 1 column and four rows
par(mfrow=c(4,1))
#Plot the mean spectrum for each spectra group above
plot(mean(spec.d), lwd = 1.2, col = "black", ylab = "Absorbance", cex.lab = 1.5, cex.main = 1.5, xlab = "Wavelength (nm)", main = "Average - Dark")

plot(mean(spec.l), lwd = 1.2, col = "red", ylab = "Absorbance", cex.lab = 1.5, cex.main = 1.5, xlab = "Wavelength (nm)", main = "Average - Light")

plot(mean(spec.t), lwd = 1.2, col = "purple", ylab = "Absorbance", cex.lab = 1.5, cex.main = 1.5, xlab = "Wavelength (nm)", main = "Average - Translucent")

plot(mean(spec.w), lwd = 1.2, col = "blue", ylab = "Absorbance", cex.lab = 1.5, cex.main = 1.5, xlab = "Wavelength (nm)", main = "Average - White")
...
#raman <-matrix2SpectraObject(
  #gr.crit = c("Translucent", "White", "Dark", "Light" ),
  #gr.cols = c("purple", "blue", "black", "yellow"),
  #freq.unit ="Wavenumber (cm-1)",
  #int.unit ="Intensity",
  #descrip ="Bifacial points measurements",
  #in.file = "./analysis/data/derived_data/raman_baseline_corrected_transposed_20210908.csv",
  #sep = ";",
  #dec = ",",
 # chk = TRUE,
 # out.file = "raman")

#surveySpectra(raman, by.gr = TRUE , method = "sem", main = "mean")

r.dark <- Points.raman %>% 
  filter(hue == "Dark")

spec.rd <- 
  as_spectra(r.dark, name_idx = 1, meta_idxs = 2:28)

r.light <- Points.raman %>% 
  filter(hue == "Light")

spec.rl <- 
  as_spectra(r.light, name_idx = 1, meta_idxs = 2:28)

r.translucent <- Points.raman %>% 
  filter(hue == "Translucent")

spec.rt <- 
  as_spectra(r.translucent, name_idx = 1, meta_idxs = 2:28)

r.white <- Points.raman %>% 
  filter(hue == "White")

spec.rw <- 
  as_spectra(r.white, name_idx = 1, meta_idxs = 2:28)


par(mfrow=c(4,1))

plot(mean(spec.rd), lwd = 1.2, col = "black", ylab = "Intensity", cex.lab = 1.5, cex.main = 1.5, xlab = "Wavenumber (cm-1)", main = "Average - Dark")

plot(mean(spec.rl), lwd = 1.2, col = "red", ylab = "Intensity", cex.lab = 1.5, cex.main = 1.5, xlab = "Wavenumber (cm-1)", main = "Average - Light")

plot(mean(spec.rt), lwd = 1.2, col = "purple", ylab = "Intensity", cex.lab = 1.5, cex.main = 1.5, xlab = "Wavenumber (cm-1)", main = "Average - Translucent")

plot(mean(spec.rw), lwd = 1.2, col = "blue", ylab = "Intensity", cex.lab = 1.5, cex.main = 1.5, xlab = "Wavenumber (cm-1)", main = "Average - White")
```
