...
metadata.csv <-
  read.csv2("./analysis/data/raw_data/metadata_20220119.csv", sep = ";", header = TRUE, na = c("", "NA", "NULL"))

#Import xrf data, set empty fields to NA
xrf.csv <-
  read.csv2("./analysis/data/raw_data/XRF/xrf_raw_data_20220127.csv", sep = ";", dec = ".", header = TRUE, check.names = FALSE, na = c("","NA","NULL"))
...
raman.csv <-
  read.csv2("./analysis/data/raw_data/RAMAN/raman_samples_data_non_treated_20220127.csv", sep = ";", dec = ",", header = TRUE, check.names = FALSE, na = c("","NA","NULL",NULL))
...
plot(mean(spec.rd), lwd = 1.2, col = "black", ylab = "Intensity", cex.lab = 1.5, cex.main = 1.5,
     xlab = parse(text=paste("Wavenumber (cm^-1)")), main = "Average - Dark")
...
#bind the basic fviz plot for PC 1 and 3, with metadata for visualization, and use as basis for a more customizeable plot in ggpplot
#In order to be able to filter out certain points that have the 700-800 wavenumber peaks and give them a red circle in a second geom_point statement this 
#has to be done outside of ggplot statement
r.data2 <- 
  cbind(basic_plot2$data, Points.raman[, c(1,8,10)])

#Set up the score plot and 
raman.p2 <- 
  r.data2 %>%  
  ggplot(aes(x=x, y=y, 
           shape = material, 
  ...
  guides(shape = guide_legend(override.aes = list(fill = "black"), 
         ...
         fill = guide_legend(override.aes = list(shape = 22,
                                                 ...
                                                 size=3),
                             title.position="top", title.hjust = 0.5)) +
  #Filter out points with 700 - 800 wavenumber peaks and give them a red circle and slightly increase their size for better identification
  geom_point(data = r.data2 %>% 
               filter(sample_id == 6 | sample_id == 34 | sample_id == 42 | sample_id == 127 | sample_id == 445 | sample_id == 482 | sample_id == 491 | sample_id == 494 | sample_id == 495 | sample_id == 501 |sample_id == 502 | sample_id == 503 | sample_id == 504), 
             pch=21,
             size=4, 
             colour ="red") +
  theme(legend.position = "none")
...
Although no distinct enough groupings were observed as to make classification possible, a number of structural and compositional characteristics were identified. Despite the Raman PCA lacking descriptive power, due to the few features observed in the spectra, the presence of the “graphite peaks” may present an important piece of information in the identification of the potential geological origin of the material. Interestingly, mentions of graphitic quartzite can be found in an early 20th century description of an excursion to Storfjället, Tärna parish, by the Geological Society [@geologiska25], but otherwise does not seem to be a frequently used term in the modern day. 

Naturally, identifying the geological origin does not necessitate a direct link between that point and the procurement of the material, and further studies are necessary in order to infer resource management strategies from this evidence. The characterization of quartz and quartzite is fundamental, however, if questions of resource procurement, trade and exchange and human mobility are to be developed. Being able to distinguish between different types of quartz and quartzite would enable various spatial analyses and modelling of distribution patterns, regardless of whether the geological source is known or not.  Moving forwards, the answer may lie in the field of multi-block analysis, where the potential for joint relations between separate blocks of data can be tested.  
