...
```{r nir-spectra, fig.width=20, fig.height=8, fig.cap= "Average NIR spectra by hue. Darker samples feature higher overall absorbance, as well as less pronounced peaks, when compared to lighter/translucent samples."}

#Aggregate raw NIR data by inferred hue
nir.agg <- 
  aggregate(nir.csv[, 28:2178], list(nir.csv$hue), mean)

#Transpose the data in preparation for working in ChemoSpec 
# make row names unique & sorted correctly in increasing order
# by appending numbers in running order
nir.t <- nir.agg %>%
  dplyr::mutate(Group.1 = paste(seq(1, n()),
                         Group.1 ,
                      sep = "_")) %>%
  gather(row, value, -Group.1 ) %>% 
  spread(Group.1 , value)

# strip away the appended numbers from the newly created column names
names(nir.t) <- sapply(strsplit(names(nir.t), "_"), function(x){x[2]})
#Change first column name to 'freq'  
colnames(nir.t)[1] <- "freq"

#Set the first column containing frequencies to numeric
nir.t[,1] <- sapply(nir.t[,1], as.numeric)
#Sort dataframe by frequencies asc.
nir.t <- nir.t[order(nir.t$freq),] 
#Write to csv file, since ChemoSpec doesn't work with dataframes
write.csv2(nir.t,"./analysis/data/derived_data/nir_averaged_by_hue.csv", row.names = FALSE)

#Read exported csv and create new file for chemospec
nir <-matrix2SpectraObject(
  gr.crit = c("Translucent", "White", "Dark", "Light" ),
  gr.cols = c("purple", "blue", "black", "red"),
  freq.unit ="Wavelength (nm)",
  int.unit ="Absorbance",
  descrip ="Bifacial points measurements",
  in.file = "./analysis/data/derived_data/NIR_averaged_by_hue.csv",
  sep = ";",
  dec = ",",
  chk = TRUE,
  out.file = "nir")

#Plot spectra averaged by inferred hue
Dark <- plotSpectra(nir,
            which = 1,
            yrange = c(1.05,1.2),
            lab.pos = 0.0,
            main = "Average spectra - Dark", font.lab = 2)
#Plot spectra averaged by inferred hue
Light <- plotSpectra(nir,
            which = 2,
            yrange = c(0.7,1),
            lab.pos = 0.0,
            main = "Average spectra - Dark", font.lab = 2)
#Plot spectra averaged by inferred hue
Translucent <- plotSpectra(nir,
            which = 3,
            yrange = c(1.05,1.2),
            lab.pos = 0.0,
            main = "Average spectra - Dark", font.lab = 2)
#Plot spectra averaged by inferred hue
White <- plotSpectra(nir,
            which = 4,
            yrange = c(1.05,1.2),
            lab.pos = 0.0,
            main = "Average spectra - Dark", font.lab = 2)
...
```{r raman-spectra, fig.width=20, fig.height=10, fullwidth = TRUE, fig.cap="Mean spectrum of the raman data grouped by their hue. The *quartz peak* at 460 cm-1 is present in all four groups, but the *graphite peaks* at 1 287  cm-1 (D band) and 1 598 cm-1 (G band) mainly appear in the light and dark groups"}
...
```
<!-- #Identify file with raman spectra and import
file <- '../quantifyingquartzite/analysis/data/derived_data/raman_baseline_corrected_transposed_20210826.csv'
spectra <- read.csv2(file, sep = ";", dec = ",", header = TRUE)

#Prepare function for plotting spectra
...
plot_multi(spectra, y=120, cols = rainbow(ncol(spectra)), type='l', lwd=3,
           xlab = "Wavenumber (cm-1)", ylab = "Intensity") -->
...
```-->
```{r raman-pca, fig.width=20, fig.height=15, fig.cap="Average spectra of raman measurements sampled on grey-to-dark quartzite featuring both the *quartz peak* at 460 cm-1, as well as the *graphite peaks* at 1 287  cm-1 (D band) and 1 598 cm-1 (G band)"}
