...
#Set the legend for the combined figure of both score plots
comb1 <-
...
#Call on the finished figure and layout in 2 rows
comb1 +
...
raman.p2 <- 
  ...
  guides(shape = guide_legend(override.aes = list(fill = "black"), 
         ...
         fill = guide_legend(override.aes = list(shape = 22,
                                                 ...
                                                 size=3),
                             title.position="top", title.hjust = 0.5)) +
  #Filter out points with 700 - 800 wavenumber peaks and give them a red circle for easier identification
...
#Filter out Si values above 50%
high_si <- 
  Points.xrf %>% 
    filter(`14Si` > 50)

#Prepare a calculation of percentage measurements above 50% to call on in text
high_si <- 
  round(length(high_si$`14Si`) / length(Points.xrf$`14Si`) * 100)
